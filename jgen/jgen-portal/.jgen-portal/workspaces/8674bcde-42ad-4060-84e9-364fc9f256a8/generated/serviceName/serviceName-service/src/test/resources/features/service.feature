Feature: Tests the serviceName Service using a REST client.
 
  Scenario: Save the serviceName first.
    ...
    And I POST a REST request to URL "/serviceName" with payload
    """
    {
  ...
  Scenario: Retrieve the saved serviceName
    Given that "entity" equals "serviceName"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a serviceName using an ID that already is determined
